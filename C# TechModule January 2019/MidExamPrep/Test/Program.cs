...
namespace GrainsOfSand
{
    class Program
    {
            ...
            while (command != "Mort")
            {
                ...
                switch (operation)
                {
                    ...
                    case "Increase":
                        ...
                        int increase = grains.FirstOrDefault(v => v >= value);

                        if (GetFirstValue(grains, value) == null)
                        {
                            increase = grains[grains.Count - 1];
                        }
        ...
        private static int? GetFirstValue(List<int> grains, int value)
        {
            int? result = null;

            foreach (var v in grains)
            {
                if (v == value)
                {
                    result = v;
